...
const auth = async (req, res) => {
  try {
    ...
    if(chat){
      ...
      if(!room) {
        room = await roomsService.createRoom({chatId: chat.id,name: chat.title}, {userId: user.id, name: userFinal.name, isAdmin: true, grantedBy: null, chatMember: true, payer: user.id})
      }
      roomId = room?.id || null
    }
